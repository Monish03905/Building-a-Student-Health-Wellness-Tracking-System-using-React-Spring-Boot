...
type Language = 'en' | 'hi' | 'te' | 'ta' | 'ml' | 'kn' | 'fr' | 'de';

const SUPPORTED_LANGUAGES: { code: Language; name: string }[] = [
...
const RESPONSES: Record<Language, Record<string, string>> = {
  ...
  te: {
    ...
    sad: "మీరు కష్టకాలంలో ఉన్నట్లు అనిపిస్తోంది. వెల్నెస్ సెంటర్ మీ కోసం ఎప్పుడూ తెరిచే ఉంటుంది.",
  ...
  ml: {
    ...
    sad: "നിങ്ങൾ പ്രയാസകരമായ സമയത്തിലൂടെയാണ് കടന്നുപോകുന്നത്. വെൽനസ് സെന്റർ നിങ്ങൾക്കായി എപ്പോഴും തുറന്നിരിക്കും.",
    hello: "ഹലോ! നിങ്ങളുടെ ആരോഗ്യകാര്യത്തിൽ എനിക്ക് എങ്ങനെ സഹായിക്കാനാകും?",
    thanks: "സ്വാഗതം! ആരോഗ്യത്തോടെയിരിക്കുക."
  },
  kn: {
    greeting: "ನಮಸ್ಕಾರ! ನಾನು ಆರಾ. ಇಂದು ನೀವು ಹೇಗಿದ್ದೀರಿ?",
    default: "ನಾನು ಕೇಳುತ್ತಿದ್ದೇನೆ. ಅದರ ಬಗ್ಗೆ ಇನ್ನಷ್ಟು ಹೇಳಬಲ್ಲಿರಾ?",
    ...
    sleep: "ನಿದ್ರೆ ಬಹಳ ಮುಖ್ಯ. ಮಲಗುವ 30 ನಿಮಿಷಗಳ ಮೊದಲು ಮೊಬೈಲ್ ಬಳಸಬೇಡಿ.",
    sad: "ನೀವು ಕಷ್ಟದ ಸಮಯವನ್ನು ಎದುರಿಸುತ್ತಿದ್ದೀರಿ ಎಂದು ತೋರುತ್ತಿದೆ. ವೆಲ್ನೆಸ್ ಸೆಂಟರ್ ನಿಮಗಾಗಿ ಯಾವಾಗಲೂ ತೆರೆದಿರುತ್ತದೆ.",
...
const AIChatbot: React.FC<AIChatbotProps> = ({ onClose }) => {
  ...
  const [messages, setMessages] = useState([
    { role: 'ai', text: RESPONSES['en'].greeting }
  ]);
  ...
  useEffect(() => {
    ...
    if (messages.length === 1 && messages[0].role === 'ai') {
        setMessages([{ role: 'ai', text: RESPONSES[language].greeting }]);
    }
  }, [language]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      // Simple keyword matching (works best for English, but structure allows for expansion)
      // For a real app, use an LLM API. Here we simulate basic intent detection.
      
      const isAnxious = lowerInput.includes("anxious") || lowerInput.includes("stress") || lowerInput.includes("worried") || lowerInput.includes("tension") || lowerInput.includes("panic");
      const isDiet = lowerInput.includes("diet") || lowerInput.includes("food") || lowerInput.includes("eat") || lowerInput.includes("meal") || lowerInput.includes("nutrition");
      const isSleep = lowerInput.includes("sleep") || lowerInput.includes("tired") || lowerInput.includes("insomnia") || lowerInput.includes("wake");
      const isSad = lowerInput.includes("sad") || lowerInput.includes("depress") || lowerInput.includes("cry") || lowerInput.includes("help") || lowerInput.includes("lonely");
      const isHello = lowerInput.includes("hi") || lowerInput.includes("hello") || lowerInput.includes("hey") || lowerInput.includes("namaste") || lowerInput.includes("vanakkam");
      const isThanks = lowerInput.includes("thank") || lowerInput.includes("thx");

      if (isAnxious) response = currentRes.anxious;
      else if (isDiet) response = currentRes.diet;
      else if (isSleep) response = currentRes.sleep;
      else if (isSad) response = currentRes.sad;
      else if (isHello) response = currentRes.hello;
      else if (isThanks) response = currentRes.thanks;
  ...
  return (
    <div className="fixed bottom-20 right-4 w-80 h-96 bg-white rounded-2xl shadow-2xl border border-indigo-100 flex flex-col overflow-hidden z-40 animate-in slide-in-from-bottom-10">
      <div className="bg-indigo-600 p-4 text-white flex justify-between items-center relative">
        ...
        <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="hover:bg-indigo-500 p-1 rounded transition-colors"><X size={18} /></button>
        </div>

        {/* Language Menu */}
        {showLangMenu && (
            <div className="absolute top-12 right-2 bg-white text-slate-800 rounded-lg shadow-xl border border-slate-100 py-2 w-32 z-50 text-sm">
                {SUPPORTED_LANGUAGES.map(lang => (
                    <button
                        key={lang.code}
                        onClick={() => { setLanguage(lang.code); setShowLangMenu(false); }}
                        className={`w-full text-left px-4 py-2 hover:bg-indigo-50 ${language === lang.code ? 'font-bold text-indigo-600' : ''}`}
                    >
                        {lang.name}
                    </button>
                ))}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50" ref={scrollRef}>
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-3 rounded-lg text-sm ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white border text-gray-700 rounded-bl-none shadow-sm'}`}>
              {m.text}
            </div>
          </div>
        ))}
      ...
      <div className="p-3 bg-white border-t flex gap-2">
        <input 
          ...
          placeholder={language === 'en' ? "Ask anything..." : "Type here..."}
          className="flex-1 text-sm border rounded-full px-3 py-2 focus:outline-none focus:border-indigo-500"
        />
        <button onClick={handleSend} className="bg-indigo-600 text-white p-2 rounded-full hover:bg-indigo-700">
          <Send size={16} />
        </button>
      </div>
    </div>
